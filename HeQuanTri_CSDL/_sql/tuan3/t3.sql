...
as (select sod.ProductID, p.Name, count(sod.SalesOrderID) as CountOfOrderID
	, sod.UnitPrice*sod.OrderQty as SubTotal
	from Production.Product as p
	join Sales.SalesOrderDetail as sod on p.ProductID=sod.ProductID
	join Sales.SalesOrderHeader as soh on sod.SalesOrderID=soh.SalesOrderID
	where (sod.UnitPrice*sod.OrderQty)>10.000
	and datepart(quarter, soh.OrderDate)=1 and year(soh.OrderDate)=2008
	group by sod.ProductID, p.Name, sod.UnitPrice, sod.OrderQty
...
select top 5 * from dbo.vw_CustomerTotals
